...
public abstract class NPCDataStorage {
    public static record ModelData(UUID uuid) {
        ...
        public static final Codec<ModelData> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            Codec.STRING.xmap(UUID::fromString, UUID::toString).fieldOf(UUID_KEY).forGetter(ModelData::getUuid)
        ).apply(inst, ModelData::new));

        public static final PacketCodec<ByteBuf, ModelData> PACKET_CODEC = PacketCodecs.codec(ModelData.CODEC);
    }
    ...
    public static record NPCData(String model_id, String dialogue_id, Mode mode) {
        ...
        public static final Codec<NPCData> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            Codec.STRING.fieldOf(MODEL_ID_KEY).forGetter(NPCData::model_id),
            Codec.STRING.fieldOf(DIALOGUE_ID_KEY).forGetter(NPCData::dialogue_id),
            Mode.CODEC.fieldOf(MODE_KEY).forGetter(NPCData::mode)
        ).apply(inst, NPCData::new));

        public static final PacketCodec<ByteBuf, NPCData> PACKET_CODEC = PacketCodecs.codec(NPCData.CODEC);
    }
    ...
    public DialogueData getDialogue(BasicNPC npc) {
        NPCData npc_data = this.getNPCMap().getOrDefault(npc.getID(), NPC_NOT_FOUND);

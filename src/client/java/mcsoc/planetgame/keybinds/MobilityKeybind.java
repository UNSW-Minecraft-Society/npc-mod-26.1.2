...
public class MobilityKeybind {
    private MobilityKeybind() { /* delete */ }

    private static KeyBinding grav_keybind;

    public static void registerBindings() {
        grav_keybind = KeyBindingHelper.registerKeyBinding(new KeyBinding(
            "mcsoc.planetgame.third_ability_keybind", // The translation key of the keybinding's name
    ...
    public static void registerBindEvents() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (grav_keybind.wasPressed()) {
                ClientPlayNetworking.send(new TriggerThirdAbilityC2SPayload());
            }
        });
    }
